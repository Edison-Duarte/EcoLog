...
# 2. Inicialização dos Dados (Carrega do arquivo ao abrir)
...
with st.expander("➕ Registrar Coleta de Resíduos", expanded=True):
    ...
    if st.button("Salvar Registro", use_container_width=True):
        if peso_in > 0:
            ...
            # Concatena e salva no arquivo físico
...
# 6. GRÁFICO (ABRE AUTOMATICAMENTE SE HOUVER DADOS)
if not st.session_state.db.empty:
    ...
    with st.expander("⚙️ Gerenciar / Excluir Registros"):
        ...
        if st.button("🗑️ Apagar Selecionados", use_container_width=True):
            ...
            salvar_dados(st.session_state.db) # Atualiza o arquivo após deletar
            st.rerun()
else:
    st.info("Aguardando o primeiro registro para gerar o gráfico.")
